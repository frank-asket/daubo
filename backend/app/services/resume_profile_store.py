"""Persist résumé-derived skills + context; read from DB for dashboards and agents."""
...
async def persist_resume_profile_signals(
    ...
    """Extract skills + context and save on the user_resumes row. Returns None if AI is unavailable."""
...
def signals_from_row(row: UserResume | None) -> tuple[ResumeProfileSignals | None, bool]:
    """Return (signals, stale) when stored hash matches current resume text."""
...
async def get_or_refresh_resume_profile_signals(
    ...
    *,
    ...
    """Read persisted profile or re-extract when stale or forced."""
    if not (settings.openrouter_api_key or "").strip():
        raise ValueError("OPENROUTER_API_KEY is not configured")
    result = await session.execute(select(UserResume).where(UserResume.clerk_user_id == clerk_user_id))
    row = result.scalar_one_or_none()
    if row is None or not (row.content_text or "").strip():
        raise ValueError("Add your résumé first — we extract skills and context from it.")
